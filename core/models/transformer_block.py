...
class TransformerDecoderBlock(nn.Module):
    def __init__(self, d_model: int, num_heads: int, ff_dim: int | None = None, activation: str = "gelu", dropout: float = 0.1, bias: bool = True, use_flash_attention: bool = False, max_seq_len: int = 8192):
        super().__init__()
        if d_model <= 0:
        ...
        self.ln1 = LayerNorm(d_model=d_model, bias=bias)
        ...
        self.ln2 = LayerNorm(d_model=d_model, bias=bias)
    ...
    def forward(self, x: torch.Tensor, past_kv=None, use_cache: bool = False):
        ...
        if use_cache:
            attn_output, present_kv = self.attn(attn_input, past_kv=past_kv, use_cache=True)
        else:
