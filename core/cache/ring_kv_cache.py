...
class RingKVCache:
    def __init__(self, batch_size: int, num_heads: int, head_dim: int, max_seq_len: int, device=None, dtype=None):
    ...
    def append(self, new_keys: torch.Tensor, new_values: torch.Tensor):
        ...
        for i in range(seq_len):
            self.keys[:, :, self.write_pos, :] = new_keys[:, :, i, :]
            self.values[:, :, self.write_pos, :] = new_values[:, :, i, :]
